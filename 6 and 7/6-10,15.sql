...
--10 How many tutors have a status of Temp Stop? Which tutors are active?
select count(*) "TempStop" from tutor where status = 'Temp Stop';
select TUTOR_ID , CERT_DATE ,STATUS  from tutor where status != 'Temp Stop';

--11 What are the TutorIDs for tutors who have not yet tutored anyone?
...
--14 How long had each student studied in the adult literacy program?
select enddate - startdate from match_history where enddate is not null;

--15 What is the average length of time a student stayed (or has stayed) in the program?
select avg(enddate - startdate) from match_history where enddate is not null;